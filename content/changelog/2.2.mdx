...
In service to our super-secret data explorer we've added a new warm color theme that covers the entire app. New CodeMirror themes and swapped Prism for Lezer so all our static hightlighting uses the same CodeMirror setup. Find it in settings to try out the dark and light modes.
