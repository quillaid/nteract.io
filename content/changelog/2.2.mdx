...
published: false
heroImage: "https://img.runt.run/2026/06/01/ce252905c39d.png" # placeholder, swap for a real screenshot or clip
...
The headline is for agents. A runtime no longer has to be a thing you create, name, and clean up. An agent can spin one up on demand, run against it, and let it close when the work wraps. The second story kills a long-standing papercut: adding a package to a conda environment used to mean a kernel restart and a lost session. Not anymore. The third you'll see everywhere, a warm Cream theme that covers the whole app, editor included.

## Ephemeral notebooks for agents

`create_notebook` now defaults to ephemeral. The notebook lives only in memory, with no file on disk and no leftover state to sweep up. An agent grabs a runtime, does its work, and the session closes when it's done, more like a regular REPL than a document you have to save. When something is worth keeping, `save_notebook(path)` writes it out. Until then there's nothing to name and nothing to garbage-collect.
