...
title: "Hidden Secrets, Sift DataFrames"
summary: "The nteract launcher is now the default, bringing rich Python tracebacks, DataFrames in the Sift explorer, and secrets kept out of your outputs."
highlights:
  - "The nteract launcher runs by default now (legacy launcher is opt-out)"
  - "Python errors render as rich, structured tracebacks"
  - "DataFrames open in Sift, an interactive crossfilter explorer"
...
published: false
heroImage: "https://img.runt.run/2026/06/01/283f94f57b2f.png" # placeholder, swap for a real screenshot or clip
# heroVideo: "https://…"   # TODO: a short demo clip (autoplays muted/looped)
---

The headline of 2.5 isn't in the commit list. It's one flag.

The **nteract launcher** is now the default. The launcher that used to be opt-in runs for everyone, with a legacy-launcher escape hatch if you need it. That single flip is the story, because the launcher is what makes three things real in every notebook.

## Rich tracebacks

Python errors stop being a wall of ANSI text. The launcher emits a structured traceback that nteract renders frame by frame: your code split from library internals, source context and line numbers inline, the noise filtered out.

## DataFrames open in Sift

Return a DataFrame and it renders in **Sift**, an interactive crossfilter explorer. Histograms and category bars live in the column headers, you brush-filter across columns at once, and it stays smooth well past 100k rows (Arrow IPC and WASM underneath). No `dx` package, nothing to import.

## Secrets stay out of outputs

2.5 also pulls your login-shell environment into kernels, so the `PATH` and variables you actually have set are there. Redaction is the safety valve: before any output leaves the kernel, values that look like secrets are scrubbed to `[redacted env]`. Your shell's secrets are available to your code, but they don't leak into a shared notebook or an agent's view. On by default, per-kernel opt-out.
