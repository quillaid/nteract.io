...
title: "Tracebacks You Can Read, Environments That Find Themselves"
summary: "Python errors render frame by frame, and your project's environment file becomes the kernel without any setup."
highlights:
  - "Rich tracebacks: errors render frame by frame with source context, your code split from library internals"
...
Two things get less annoying in 2.3: reading errors, and getting the right environment.

Both used to need a person in the loop. A traceback was a wall of text you scanned by hand. An environment was something you set up before the notebook would run the way your project expected. 2.3 takes the manual step out of both.

## Rich tracebacks

When your code raises, nteract renders the traceback frame by frame instead of dumping a block of ANSI text. Your code is split from library internals, source context shows inline with the failing line highlighted, and deep stacks like a `RecursionError` collapse to head and tail so the payload stays readable. The kernel-launcher builds a structured payload and the notebook renders it, with one hard rule: if anything in that path fails, you still get the plain traceback. You never lose the error.

## Environments that find themselves

Open a notebook next to a `pyproject.toml`, a `pixi.toml`, or an `environment.yml` and nteract picks it up. Project-file detection runs through the daemon, resolves the dependencies your project declares, and launches the kernel against them. No `dx` step, no manual environment selection. `pixi` is now a first-class manager alongside `uv` and `conda`, and resolved environments get captured into notebook metadata so a later open can cache-hit instead of re-solving.
