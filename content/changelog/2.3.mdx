...
summary: "Open a notebook next to a project file and nteract resolves the environment and launches the kernel for you. No setup step."
highlights:
  - "Project files drive the kernel: pyproject.toml, pixi.toml, and environment.yml are detected and used automatically"
  - "pixi joins uv and conda as a first-class environment manager"
  - "Prewarmed environment pools and captured-env caching mean a second open is a cache hit, not a re-solve"
...
published: false
heroImage: "https://img.runt.run/2026/06/01/6b724f5f4357.png" # placeholder, swap for a real screenshot or clip
# heroVideo: "https://…"   # TODO: a short demo clip (autoplays muted/looped)
---

2.3 is the environment release. Getting the right Python set up used to be a step you did before the notebook would run the way your project expected. 2.3 takes that step out.

## Environments that find themselves

Open a notebook next to a `pyproject.toml`, a `pixi.toml`, or an `environment.yml` and nteract picks it up. Detection walks up from the notebook directory, closest file wins, and the daemon resolves the dependencies your project declares and launches the kernel against them. No manual environment selection, no separate setup. The project file is the environment.

## pixi joins uv and conda

`pixi` is now a first-class manager alongside `uv` and `conda`, so all three project-file flavors resolve natively. `pyproject.toml` goes through `uv`, `pixi.toml` through `pixi`, `environment.yml` through `conda`. The `pixi` path talks to `rattler` directly instead of shelling out, so there's no `pixi` binary to install first.

## Prewarmed pools and cached environments

The daemon keeps a pool of prewarmed environments so a kernel launch claims a ready one instead of building from cold. Once an environment resolves, it gets captured into notebook metadata and the pool env is promoted to an inline cache. The second time you open that notebook, it's a cache hit, not a re-solve.

## Native Intel Mac builds

The release pipeline now ships `x86_64` macOS binaries next to the Apple Silicon ones. Intel Macs get a native build of the desktop app and the `runt` CLI, no Rosetta translation in the path.

## Rich tracebacks, behind a flag

For the curious: 2.3 also lands the rich traceback renderer that splits your code from library internals and shows source context inline. It ships behind a feature flag, opt-in only. Enable it if you want to try it early. 2.5 flips it on for everyone by default.
