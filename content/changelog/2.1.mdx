...
title: "Agents Always On, Pixi In The Box"
summary: "The runtime agent is on for every notebook now. Process-isolated. Pixi joins the defaults. Outputs render rich and read clean for agents."
highlights:
  - "Agent mode is unconditional and runs in its own isolated process"
  - "Pixi is a first-class Python environment, alongside uv and conda"
  - "Add packages from the notebook with pixi add / uv add and inline deps"
...
2.1 is where the agent stops being a flag.

For a few releases the runtime agent was opt-in, gated behind a Feature Flags toggle. In 2.1 it goes unconditional and moves into its own process. The agent runs as a sync-only subprocess that drives execution over the shared CRDT, the same document your notebook UI reads from. So a human typing in a cell and an agent running cells see one consistent state, and a crash in agent land can't take the notebook with it. Around that core shift, 2.1 also makes Pixi a real environment option and fills out the output renderers.

## Agent mode is always on, and isolated

The agent is no longer something you switch on. Every notebook gets one, and it lives in a separate process that talks to the daemon as a Unix-socket peer. Execution flows through the `RuntimeStateDoc` CRDT keyed by execution id, so the agent and the editor never disagree about what ran or what came back. Kernel restarts and execution provenance are tracked agent-side, and `agent_mode` persists in the settings doc so it survives daemon restarts.
...
## Rich outputs, and agents that can read them

The output stack moves to a React renderer with daemon-served plugins. Plotly, Vega and Vega-Lite, Leaflet-backed GeoJSON, LaTeX through KaTeX, plus audio, video, PDF, and JavaScript all render natively now. The other half of this is for agents: every visual output also synthesizes a `text/llm+plain` summary, so an agent looking at a Plotly chart or a GeoJSON map gets a readable description instead of a blob of base64.
