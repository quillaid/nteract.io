...
title: "TODO — write the headline (the payload, not 'v2.1 released')"
summary: "TODO — one sentence describing this release to a notebook user."
highlights:
  - "replace vanilla DOM renderer with React + renderer plugins"
  - "add async-rust-lsp plugin for tokio mutex diagnostics"
  - "reuse pool envs for inline deps instead of rebuilding"
  - "daemon-served renderer plugins for plotly, vega, leaflet"
tags:
  - "Outputs"
  - "Agents"
  - "Collaboration"
