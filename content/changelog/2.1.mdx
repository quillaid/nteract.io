...
summary: "This begins repr llm. Plotly, Vega, Leaflet, and more are supported for both humans and agent views. Native rendering for humans, text/plain+llm for agents."
highlights:
...
This release brings rich outputs for people and agents alike. Now with Pixi support!

## Outputs for Humans

<!-- TODO: Show plotly, vega, leaflet, latex -->

![](https://img.runt.run/2026/06/01/4d15812b943d.png)
...
Until now, rich outputs were a dead end for agents. They either had to sift through large plotly html, get no visibility of widgets, or recieve massive base64 encoded gunk. 2.1 marks the start of the repr-llm crate. Alongside the visual representation, every rich output also synthesiszes a `text/llm+plain` summary. When an agent connects via the nteract mcp, they see a text-readable description of what the output shows. If an output has no readable representation at all, the agent at least sees a note that something visual was produced.

## Pixi is a first-class environment

Pixi joins uv and conda as a default Python environment. Within 2.1, it's wired end to end: `pixi run` kernel launch, real pool warming against managed `pixi.toml` envs, `[tool.pixi]` detection, drift checks, and a dependency panel in the header that shows what's declared. You can add packages straight from the notebook UI, rely on your warming pool, and even use PEP 723 script metadata.
