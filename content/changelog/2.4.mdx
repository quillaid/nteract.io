...
title: "Runtimed in Node, a REPL for Pi Agents"
summary: "@runtimed/node opens the daemon behind the desktop app to any Node runtime, and @nteract/pi uses it to give pi.dev agents a real persistent Python REPL. Driveable from JavaScript. DataFrame output grows up alongside."
highlights:
  ...
  - "DataFrames render with rich types, inline images, and Arrow IPC under the hood"
  - "Outputs get a mode strip: compact, expanded, or focused per cell"
tags:
  - "Runtime"
  - "Agents"
  - "Outputs"
...
published: false
heroImage: "https://img.runt.run/2026/06/01/bc866908d0f8.png" # placeholder, swap for a real screenshot or clip
# heroVideo: "https://…"   # TODO: a short demo clip (autoplays muted/looped)
---

Eight patch releases folded into one. The throughline is one new package: the runtime that powers the desktop app is now embeddable from Node.

## The runtimed daemon, now from Node

`@runtimed/node` is the Node.js binding for `runtimed`, the same daemon behind the nteract desktop app. Anything Node, Bun, or another CommonJS runtime can now talk to the local daemon directly: `createNotebook`, run and queue cells, read outputs, sync dependencies, save notebooks, and open a live session in the desktop app. It is a thin JavaScript wrapper over a native binding, with the platform `.node` shipped as an optional install. The point is that the runtime stops being desktop-only. The persistent kernels, hot dependency sync with `uv`, and warm environment pools that the app relies on are all reachable from a script.
...
`@nteract/pi` is the first thing built on `@runtimed/node`, and it is the reason this matters now. It gives a pi.dev agent a persistent, notebook-backed Python REPL through four tools: `python_repl` runs code in an IPython kernel whose variables, imports, and figures survive across turns, `python_add_dependencies` installs packages mid-session with no restart, `python_save_notebook` exports the session to an `.ipynb` you can open in nteract or Jupyter, and `/python-reset` starts clean. Install it with `pi install npm:@nteract/pi`. Because it rides the daemon, environments stay warm per working directory, so there are no cold starts, and the state an agent builds up is the same runtime the desktop app can open and inspect.

## DataFrames that describe themselves

DataFrame output grew up this cycle. Tables now travel as Arrow IPC payloads with a manifest, so the frontend reads the same typed columns the kernel produced. HuggingFace rich types flow end to end, which means an image column from a parquet dataset renders inline in the table instead of as a blob of bytes. Long text rows collapse by default and expand on click. The manifest also carries precomputed hints, so an agent reading the output gets a useful summary without pulling the whole frame.

## Output modes, per cell

Each output now has a mode strip: compact, expanded, or focused. Compact keeps a busy notebook scannable. Focused hands the output its own scroll and sizing, which is what makes a large table or an interactive iframe usable inline. Focus is magnetic now, it sticks until you click elsewhere instead of dropping the moment your pointer leaves. Small outputs lose their chrome entirely.
