...
title: "A REPL for Agents, DataFrames That Read Themselves"
summary: "nteract ships a notebook-backed Python REPL for coding agents, and DataFrame output grows up. Rich types end to end. Inline images in tables. State that stays hot."
highlights:
  - "@nteract/pi gives coding agents a persistent Python REPL with zero cold starts"
  ...
  - "Kernels launch even with no internet, falling back to cached installs"
tags:
  - "Agents"
  - "Outputs"
  - "Runtime"
...
Eight patch releases folded into one. The throughline: the same runtime that powers the desktop app now shows up where agents work, and DataFrame output stopped being a flat grid of strings.

## A Python REPL for coding agents

`@nteract/pi` is a notebook-backed Python REPL built for Pi coding agents. It runs on `@runtimed/node`, the same daemon behind the nteract desktop app, so an agent gets a real IPython kernel with state that survives between turns. Variables, imports, and matplotlib figures stick around. Dependencies sync hot with `uv`, no kernel restart. There are no cold starts because the daemon keeps environments warm per working directory. The agent's tool calls return rich output too, including a terminal `DataTable` with per-column sparklines and a spinner in `In[*]` while the first token lands. Run `runt list` or `runt show` to inspect or open any session in the desktop app.

## DataFrames that describe themselves

DataFrame output grew up this cycle. Tables now travel as Arrow IPC payloads with a manifest, so the frontend and the Sift explorer read the same typed columns the kernel produced. HuggingFace rich types flow end to end, which means an image column from a parquet dataset renders inline in the table instead of as a blob of bytes. Long text rows collapse by default and expand on click. The manifest also carries precomputed hints, so an agent reading the output gets a useful summary without pulling the whole frame.

## Output modes, per cell

Each output now has a mode strip: compact, expanded, or focused. Compact keeps a busy notebook scannable. Focused hands the output its own scroll and sizing, which is what makes a large Sift table or an interactive iframe usable inline. Focus is magnetic now, it sticks until you click elsewhere instead of dropping the moment your pointer leaves. Small outputs lose their chrome entirely.
